...
var GOOGLE_LIMIT = 5;   // requests per second
var GOOGLE_TIME = 1000; // ms
var nextTime = 0;
var queue = [];
var timestamps = [];
var number = 0;
...
function limit(req)
{
  function addToTimestamps(now)
  {
    if ((now-timestamps[0]) > GOOGLE_TIME) timestamps.shift();
    timestamps.push(now);
  }
  ...
  function afterTimeout()
  {
    ...
    if (now > nextTime)
    {
      if (queue.length > 0)
      {
        ...
        nextTime = now+(GOOGLE_TIME-(now-timestamps[0]));
        setTimeout(afterTimeout, nextTime);
        return queue.shift()();
      }
    } else setTimeout(afterTimeout, nextTime-now);
  }
  ...
  function next()
  {
    ...
    if (now > nextTime)
    {
      if (timestamps.length >= GOOGLE_LIMIT)
      {
        nextTime = now+(GOOGLE_TIME-(now-timestamps[0]));
        setTimeout(afterTimeout, nextTime);
      }
